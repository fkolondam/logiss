<template>
  ...
  <aside
    ...
    :class="[
      isMobile
        ? 'inset-x-0 top-16 bottom-16 shadow-none border-l-0'
        : 'w-96 top-16 right-0 bottom-0',
  ...
  >
    ...
    <div class="flex-none">
      <div
        class="h-[4.5rem] px-4 flex items-center justify-between border-b border-gray-200/80 bg-white"
      >
        <div class="flex items-center gap-3">
          ...
          <h2 class="text-lg font-display font-bold tracking-tight text-gray-900">
    ...
    <div class="flex-1 overflow-y-auto scrollbar-elegant">
      <div class="p-4 space-y-4">
        ...
        <div v-if="activeTab === 'delivery'" key="delivery" class="space-y-3">
          <div
            class="flex items-start gap-3 p-3 rounded-xl hover:bg-gray-50/80 transition-colors duration-200 group"
          >
            <Receipt
              class="w-5 h-5 text-gray-400 mt-0.5 group-hover:text-primary-500 transition-colors duration-200"
            />
            <div>
              <label
                class="block text-xs font-medium text-gray-500 uppercase tracking-wide font-display"
                >{{ t('deliveries.table.invoice') }}</label
              >
          ...
          >
            ...
            <div>
              <label
                class="block text-xs font-medium text-gray-500 uppercase tracking-wide font-display"
                >{{ t('deliveries.table.amount') }}</label
              >
              <div class="flex items-center gap-2">
                <span class="text-sm font-display text-gray-900"
                  >Rp {{ formatNumber(delivery.amount || 0) }}</span
                >
                <span
                  class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-gray-100/80 text-gray-600 font-medium"
                >
                  <component
          ...
          >
            ...
            <div>
              <label
                class="block text-xs font-medium text-gray-500 uppercase tracking-wide font-display"
                >{{ t('deliveries.table.driver') }}</label
              >
          ...
          >
            ...
            <div>
              <label
                class="block text-xs font-medium text-gray-500 uppercase tracking-wide font-display"
                >{{ t('deliveries.table.licensePlate') }}</label
              >
        ...
        <div v-if="activeTab === 'customer'" key="customer" class="space-y-4">
          ...
          >
            ...
            <div>
              <label
                class="block text-xs font-medium text-gray-500 uppercase tracking-wide font-display"
                >{{ t('deliveries.table.customer') }}</label
              >
          ...
          >
            ...
            <div>
              <label
                class="block text-xs font-medium text-gray-500 uppercase tracking-wide font-display"
                >{{ t('deliveries.table.location') }}</label
              >
          ...
          <!-- Map -->
          <div
            v-if="delivery.coordinates"
            class="mt-4 rounded-xl overflow-hidden shadow-subtle hover:shadow-float transition-all duration-200"
          >
            <iframe
              width="100%"
              height="200"
        ...
        <div v-if="activeTab === 'proof'" key="proof" class="space-y-4">
          ...
          <div
            v-else
            class="flex flex-col items-center justify-center gap-2 text-sm text-gray-500 text-center py-8 rounded-xl border-2 border-dashed border-gray-200 hover:border-gray-300 transition-colors duration-200"
          >
            <ImageOff
              class="w-12 h-12 text-gray-300 group-hover:text-gray-400 transition-colors duration-200"
            />
