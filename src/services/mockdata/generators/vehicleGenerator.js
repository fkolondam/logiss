import { branchConfig, vehicleStatusConfig } from './branchData';
import { getWorkingDaysInRange, formatDate } from './dateUtils';
import { getRandomNumber, getRandomCoordinates } from './randomUtils';
...
const generateBaseVehicle = (plateNumber, branch, driver, coordinates) => {
  ...
  const vehicleModels = {
    'Truck': ['Toyota Hilux', 'Mitsubishi Fuso', 'Hino Dutro', 'Toyota Dyna', 'Mitsubishi Canter', 'Hino Ranger'],
  ...
  const fuelEfficiency = type === 'Truck' ? 
  ...
  return {
    ...
    status: "active",
    fuelLevel: getRandomNumber(vehicleStatusConfig.fuelLevel.startDay.min, vehicleStatusConfig.fuelLevel.startDay.max),
...
const generateDocuments = (branch) => {
  const providers = ['Asuransi Sinar Mas', 'Asuransi Jasindo', 'Asuransi Adira', 'Asuransi Allianz', 'Asuransi Astra'];
...
const generateMaintenanceHistory = (plateNumber, schedule, workingDays) => {
  ...
  const maintenanceTypes = [
    { type: 'Regular Service', description: 'Oil change, brake check', costRange: [1000000, 2000000] },
    ...
    { type: 'Brake Service', description: 'Brake system maintenance', costRange: [1500000, 2000000] }
  ];
