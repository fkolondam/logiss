import { branchConfig, expenseCategoryConfig } from './branchData';
import { getWorkingDaysInRange, formatDate } from './dateUtils';
import { getRandomAmount, generateProofImageUrl } from './randomUtils';
...
const generateExpenseData = (startDate, endDate) => {
  ...
  Object.entries(branchConfig).forEach(([branchName, branchData]) => {
    ...
    vehicles.forEach(vehicleNumber => {
      ...
      const driver = Object.values(branchData.drivers).find(d => 
        branchData.vehicles.indexOf(vehicleNumber) === branchData.drivers.indexOf(d)
      );
...
const generateDescription = (category) => {
  const descriptions = {
    ...
    'Maintenance': ['Regular service', 'Monthly maintenance', 'Routine maintenance', 'Vehicle maintenance'],
    'Vehicle License': ['License renewal', 'Registration fee', 'Vehicle registration', 'License fee'],
    'Labour': ['Labour expense', 'Workman charges', 'Labour work', 'Labour charge'],
    'Parking-Tol-Retribution': ['Toll fee', 'Parking charges', 'Toll and parking', 'Parking and toll charges']
  };
